...
| ------------------------------ | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `MCP_API_KEY`                  | _(optional — no auth without it)_                | Bearer token for REST + MCP over HTTP. Ignored in stdio mode. When unset the server starts without authentication (all requests accepted).                                                       |
...
| `VERSION`                      | `1.3.0`                                          | Reported version.                                                                                                                                                                                |
...
> instantiated in-memory for the duration of the suite (see `src/test/e2e/pg-store.e2e.test.ts`).
