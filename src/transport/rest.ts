...
export const createRestApp = async (store: Store, queue: JobQueue) => {
  ...
  const SEARCH_LIMIT = Number.parseInt(
    process.env.SEARCH_RATE_PER_MINUTE || "60",
    10,
  );
  const REINDEX_LIMIT = Number.parseInt(
    process.env.REINDEX_RATE_PER_MINUTE || "60",
    10,
  );
  ...
  app.post<{ Params: { kb: string }; Body: AddBody }>(
    "/admin/kbs/:kb/documents",
    async (req, reply) => {
      if (!auth(req, reply)) return;
  ...
  app.delete<{ Params: { kb: string; "*": string } }>(
    "/admin/kbs/:kb/documents/*",
    async (req, reply) => {
      if (!auth(req, reply)) return;
  ...
  app.get<{ Querystring: { kb: string; path: string } }>(
    "/document",
    async (req, reply) => {
      if (!auth(req, reply)) return;
  ...
  app.get<{ Querystring: { query?: string; kb?: string; top_k?: string } }>(
    "/search",
    searchLimiter,
    async (req, reply) => {
      if (!auth(req, reply)) return;
