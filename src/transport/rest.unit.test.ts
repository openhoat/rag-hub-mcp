...
describe('rest', () => {
  ...
  test('GET /search should split comma-separated kb into an array', async () => {
    mockedSearch.mockResolvedValueOnce([{ kb: 'kb', relPath: 'f.md', chunkIndex: 0, content: 'hit', score: 0.5 }])
    const res = await fetch(`${base}/search?query=hello&kb=infra,dev`, {
      headers: AUTH,
    })
    expect(res.status).toBe(200)
  ...
  test('POST /admin/kbs/:kb/documents should add with raw path and string content', async () => {
    ...
    expect(res.status).toBe(200)
    expect(mockedAdd).toHaveBeenCalledWith(expect.anything(), expect.anything(), 'docs', '../evil.md', 'hello')
  })
  ...
  test('GET /admin/status should return kb list and indexer stats', async () => {
    const res = await fetch(`${base}/admin/status`, { headers: AUTH })
    expect(res.status).toBe(200)
    const body = (await res.json()) as {
      kbs: Array<{ name: string }>
      indexer: { pending: number; processing: number; failed: number }
    }
    expect(body.kbs[0].name).toBe('kb')
    expect(body.indexer).toEqual({ pending: 0, processing: 0, failed: 0 })
  })
  ...
  test('GET /document should return extracted content', async () => {
    ...
    const res = await fetch(`${base}/document?kb=kb&path=notes%2Farch.md`, {
      headers: AUTH,
    })
    expect(res.status).toBe(200)
    const body = (await res.json()) as {
  ...
  test('GET /document should return frontmatter metadata when present', async () => {
    ...
    const res = await fetch(`${base}/document?kb=kb&path=notes%2Fintro.md`, {
      headers: AUTH,
    })
    expect(res.status).toBe(200)
    const body = (await res.json()) as {
  ...
  test('GET /document should return 404 when the document is missing', async () => {
    ...
    const res = await fetch(`${base}/document?kb=kb&path=missing.md`, {
      headers: AUTH,
    })
  ...
  test('GET /document should return 400 on invalid path', async () => {
    ...
    const res = await fetch(`${base}/document?kb=kb&path=../evil.md`, {
      headers: AUTH,
    })
    expect(res.status).toBe(400)
  })
  ...
  test('should decode URL-encoded path in DELETE document', async () => {
    ...
    const res = await fetch(`${base}/admin/kbs/docs/documents/${encoded}`, {
      method: 'DELETE',
      headers: AUTH,
    })
    expect(res.status).toBe(200)
    expect(mockedDelete).toHaveBeenCalledWith(expect.anything(), expect.anything(), 'docs', 'notes/my file.md')
  })
  ...
  test('should return 429 on /search when the rate limit is exceeded', async () => {
    ...
    try {
      ...
      try {
        let res = await fetch(`${limitedBase}/search?query=hello`, {
          headers: AUTH,
        })
        expect(res.status).toBe(200)
        res = await fetch(`${limitedBase}/search?query=again`, {
          headers: AUTH,
        })
