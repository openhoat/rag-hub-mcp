...
const searchArgs = {
  ...
  kb: z
    ...
    .optional()
    .describe(
      'Knowledge base name(s), comma-separated for multiple (eg "infra,dev"). Omit to search all.',
    ),
...
const reindexArgs = {
  force: z
    ...
    .optional()
    .describe(
      "If true, purge all chunks and files before scanning (full rebuild). Default: false.",
    ),
};

export const createMcpServer = (store: Store, queue: JobQueue): McpServer => {
  const server = new McpServer(
    { name: "rag-hub-mcp", version: VERSION },
    { capabilities: { tools: {} } },
  );

  server.registerTool(
    "rag_list_kbs",
    { description: "List available knowledge bases with stats" },
    async () => handleToolCall(store, queue, "rag_list_kbs", {}),
  );

  server.registerTool(
    "rag_list_documents",
    { description: "List documents in a knowledge base", inputSchema: kbArgs },
    async (args) => handleToolCall(store, queue, "rag_list_documents", args),
  );
  ...
  server.registerTool(
    ...
    {
      description:
        "Add a text document to a knowledge base. KB folder is created if needed.",
      inputSchema: addDocArgs,
    },
    async (args) => handleToolCall(store, queue, "rag_add_document", args),
  );
  ...
  server.registerTool(
    ...
    {
      description:
        "Retrieve the full extracted content of a document by KB and path.",
      inputSchema: readArgs,
    },
    async (args) => handleToolCall(store, queue, "rag_read", args),
  );
  ...
  server.registerTool(
    "rag_delete_document",
    {
      ...
      inputSchema: deleteDocArgs,
    },
    async (args) => handleToolCall(store, queue, "rag_delete_document", args),
  );

  server.registerTool(
    "rag_delete_kb",
    {
      description: "Delete an entire knowledge base (folder + index)",
      inputSchema: kbArgs,
    },
    async (args) => handleToolCall(store, queue, "rag_delete_kb", args),
  );
  ...
  server.registerTool(
    ...
    async (args) => handleToolCall(store, queue, "rag_reindex", args),
  );
  ...
  server.registerTool(
    ...
    async (args) => handleToolCall(store, queue, "rag_status", args),
  );
  ...
  server.registerTool(
    ...
    {
      description:
        "Show indexing queue status (pending/processing/failed jobs)",
    },
    async () => handleToolCall(store, queue, "rag_jobs", {}),
  );
...
export const handleToolCall = async (
  ...
  switch (name) {
    case "rag_list_kbs": {
      ...
      const lines = kbs.map(
        (k) =>
          `- **${k.name}**: ${k.docCount} documents, ${k.chunkCount} chunks, ${fmt(k.totalBytes)}`,
      );
      return {
        content: [
    ...
    case "rag_list_documents": {
      ...
      const lines = docs.map(
        (d) => `- **${d.relPath}** (${d.chunkCount} chunks, ${fmt(d.bytes)})`,
      );
      return {
...
${r.content}`,
    ...
    case "rag_delete_document": {
      ...
      return {
        content: [
          { type: "text", text: `Document deleted: **${kb}/${path}**` },
        ],
      };
    }
    ...
    case "rag_read": {
      ...
      if (doc === null) {
        return {
          content: [
            { type: "text", text: `Document not found: **${kb}/${path}**` },
          ],
          isError: true,
        };
      }
    ...
    case "rag_reindex": {
      ...
      const result = force
        ? await forceReindex(store, queue)
    ...
    case "rag_status": {
      ...
      const lines = [
        ...
        ...kbs.map(
          (k) =>
            `- **${k.name}**: ${k.docCount} docs, ${k.chunkCount} chunks, ${fmt(k.totalBytes)}`,
        ),
        "",
    ...
    case "rag_jobs": {
      ...
      const lines = [
        `Queue: ${stats.pending} pending, ${stats.processing} processing, ${stats.failed} failed`,
      ];
      if (failed.length > 0) {
        lines.push("", "**Failed jobs:**");
        for (const j of failed) {
          lines.push(
            `- **${j.kb}/${j.relPath}**: ${j.lastError ?? "unknown"} (${j.attempts} attempts)`,
          );
        }
      }
      return { content: [{ type: "text", text: lines.join("\n") }] };
