...
describe('contextual chunking end-to-end', () => {
  ...
  test('should enrich embeddings with a generated context without altering stored content', async () => {
    ...
    restoreFetch = stubLlmApi(
      ...
      payload => {
        chatCalls.push(payload.messages[payload.messages.length - 1] ?? { content: '' })
    ...
    store = createSqliteStore(join(mkdtempSync(join(tmpdir(), 'rag-ctx-db-')), 'rag.db'))

    writeKbDocument(root, 'kb1', 'api.md', '# Rate limits\n\nthe API v2 enforces a limit of 100 requests per minute')
    ...
    for (const c of chunks) {
      ...
      expect(c.content).not.toContain('This fragment details the API v2 rate limits.')
    }

    // 4. Search still retrieves the chunk through the enriched embedding.
    const found = await search(store, { query: 'rate limit', kb: 'kb1', topK: 5 })
