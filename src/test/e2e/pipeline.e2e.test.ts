...
import { deleteDocument, scanAll } from "../../core/ingest";
...
describe("full pipeline: disk -> scan -> extract -> chunk -> embed -> index -> search", () => {
  let root: string;
  let store: Store;
  let queue: ReturnType<typeof createSyncTestQueue>;
  ...
  const setup = () => {
    ...
    queue = createSyncTestQueue(store, root);
  };
  ...
  test("should remove a deleted file from the index", async () => {
    ...
    await scanAll(store, queue, root);
    await deleteDocument(store, queue, "kb1", "a.md", root);
  ...
  test("should return the correct chunk for a long document", async () => {
    ...
    writeKbDocument(
      ...
      Array.from(
        { length: 30 },
        (_, i) => `needle${i} filler words for paragraph ${i}`,
      ).join("\n\n"),
    );
    await scanAll(store, queue, root);
  ...
  test("should not return results from a different knowledge base", async () => {
    ...
    const filteredA = await search(store, {
      query: "shared topic",
      kb: "kbA",
      topK: 10,
    });
    ...
    const filteredB = await search(store, {
      query: "shared topic",
      kb: "kbB",
      topK: 10,
    });
