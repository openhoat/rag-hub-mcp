...
describe("REST API (real store + ingest + search)", () => {
  ...
  beforeEach(async () => {
    restoreFetch = stubEmbeddingsApi((texts) =>
      texts.map(() => unitEmbeddings(4)),
    );
    ...
    const started = await startHttpServer(await createRestApp(store, queue));
  ...
  test("should run a full KB lifecycle via the admin and search endpoints", async () => {
    ...
    // List docs
    const docsRes = await fetch(`${base}/admin/kbs/docs/documents`, {
      headers: AUTH,
    });
    ...
    // Search finds it
    const searchRes = await fetch(`${base}/search?query=brown+fox&kb=docs`, {
      headers: AUTH,
    });
    expect(searchRes.status).toBe(200);
    const searchBody = (await searchRes.json()) as {
      results: Array<{ kb: string }>;
    };
    ...
    const statusBody = (await statusRes.json()) as {
      kbs: Array<{ name: string }>;
    };
    expect(statusBody.kbs.some((k) => k.name === "docs")).toBe(true);

    // Delete the document
    const encoded = encodeURIComponent("notes/hello.md");
    const delRes = await fetch(`${base}/admin/kbs/docs/documents/${encoded}`, {
      method: "DELETE",
      headers: AUTH,
    });
    expect(delRes.status).toBe(200);

    const searchAfter = await fetch(`${base}/search?query=brown+fox&kb=docs`, {
      headers: AUTH,
    });
  ...
  test("should reindex files dropped on disk via POST /admin/reindex", async () => {
    writeKbDocument(
      process.env.KB_ROOT as string,
      "rebk",
      "ref/api.md",
      "restful apis return json responses",
    );
    const reindexRes = await fetch(`${base}/admin/reindex`, {
      method: "POST",
      headers: AUTH,
    });
    ...
    const searchRes = await fetch(`${base}/search?query=json`, {
      headers: AUTH,
    });
    const searchBody = (await searchRes.json()) as {
  ...
  test("should rebuild the index when POST /admin/reindex?force=true is used", async () => {
    ...
    const forceRes = await fetch(`${base}/admin/reindex?force=true`, {
      method: "POST",
      headers: AUTH,
    });
    ...
    const searchRes = await fetch(`${base}/search?query=marker`, {
      headers: AUTH,
    });
    const searchBody = (await searchRes.json()) as {
      results: Array<{ relPath: string }>;
  ...
  test("should delete an entire knowledge base", async () => {
    ...
    const delRes = await fetch(`${base}/admin/kbs/tmp`, {
      method: "DELETE",
      headers: AUTH,
    });
    expect(delRes.status).toBe(200);

    const docRes = await fetch(`${base}/admin/kbs/tmp/documents`, {
      headers: AUTH,
    });
  ...
  test("should read a document via GET /document", async () => {
    const addRes = await fetch(`${base}/admin/kbs/docs/documents`, {
      ...
      body: JSON.stringify({
        path: "readme.md",
        content: "read the full content here",
      }),
    });
    expect(addRes.status).toBe(200);

    const encoded = encodeURIComponent("readme.md");
    const res = await fetch(`${base}/document?kb=docs&path=${encoded}`, {
      headers: AUTH,
    });
  ...
  test("should return frontmatter via GET /document for markdown", async () => {
    ...
    const res = await fetch(
      `${base}/document?kb=docs&path=${encodeURIComponent("report.md")}`,
      { headers: AUTH },
    );
    expect(res.status).toBe(200);
    const body = (await res.json()) as {
  ...
  test("should search across multiple knowledge bases via comma-separated kb", async () => {
    ...
    const res = await fetch(`${base}/search?query=beta&kb=kbAlpha,kbBeta`, {
      headers: AUTH,
    });
    expect(res.status).toBe(200);
