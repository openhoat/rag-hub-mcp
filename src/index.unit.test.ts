...
const makeReply = (): FastifyReply & { rawBody: string | undefined } => {
  ...
  Object.defineProperty(reply, 'rawBody', { get: () => holder.value, enumerable: true })
  return reply
}
...
describe('index bootstrap helpers', () => {
  test('closeSession removes a known session and closes its transport', () => {
    ...
    sessions.set('s1', {
      ...
      transport: { close, handleRequest: vi.fn() } as unknown as StreamableHTTPServerTransport,
    })
  ...
  test('handleExistingSession forwards the request to the session transport', async () => {
    ...
    sessions.set('s1', {
      server: { connect: vi.fn() } as unknown as McpServer,
      transport: { close: vi.fn(), handleRequest } as unknown as StreamableHTTPServerTransport,
    })
  ...
  test('handleNewSession wires a new server+transport and handles the request', async () => {
    ...
    const sessions = new SessionRegistry<McpSession>(10, 1000)
    const handleRequest = vi.fn().mockResolvedValue(undefined)
    const transport = { handleRequest, close: vi.fn().mockResolvedValue(undefined) } as unknown as StreamableHTTPServerTransport
  ...
  test('registerMcpEndpoints wires MCP routes with auth, session and capacity handling', async () => {
    ...
    await registerMcpEndpoints(app, sessions, store, { createRateLimit: 100 })
    await app.ready()

    // GET /mcp lists tool names.
    const listRes = await app.inject({ method: 'GET', url: '/mcp' })
    expect(listRes.statusCode).toBe(200)
    expect(JSON.parse(listRes.body)).toMatchObject({ name: 'rag-hub-mcp' })

    // 503 when the session registry is at capacity and no session id is sent.
    sessions.set('occupied', { server: {} as McpServer, transport: {} as StreamableHTTPServerTransport })
    const capRes = await app.inject({ method: 'POST', url: '/mcp', headers: { authorization: 'Bearer test-secret-key' } })
    expect(capRes.statusCode).toBe(503)

    // 404 for an unknown explicit session id.
    const notFound = await app.inject({
      method: 'POST',
      url: '/mcp',
      headers: { authorization: 'Bearer test-secret-key', 'mcp-session-id': 'ghost' },
    })
