...
export const handleExistingSession = async (
  ...
  await entry.value.transport.handleRequest(
    request.raw,
    reply.raw,
    request.body,
  );
...
export const main = async (): Promise<void> => {
  ...
  const MCP_SESSION_CREATE_RATE_PER_MINUTE =
    env.MCP_SESSION_CREATE_RATE_PER_MINUTE;
  ...
  logger.info(
    `rag-hub-mcp v${env.VERSION} starting (${isHttpMode ? "http" : "stdio"})...`,
  );
  ...
  const sessions = new SessionRegistry<McpSession>(
    MCP_SESSION_MAX,
...
export const registerMcpEndpoints = async (
  ...
  setInterval(() => {
    ...
    if (expired.length > 0)
      logger.info(`evicted ${expired.length} stale MCP session(s)`);
  }, 60_000).unref();
};
