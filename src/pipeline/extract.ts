 ...
 * Exported so the scan producer can pre-check without reading the full file. */
export const isBinaryContent = (buf: Buffer): boolean => {
  const sample = buf.subarray(0, BINARY_SAMPLE_SIZE);
  if (sample.includes(0)) return true;
  const decoded = sample.toString("utf-8");
  return (
    Buffer.from(decoded, "utf-8").toString("hex") !== sample.toString("hex")
  );
};

const extractByExt = async (
  ext: string,
  filePath: string,
...
const extractPptx = async (filePath: string): Promise<ExtractResult> => {
  try {
    ...
    for (const name of slides) {
      ...
      const xml = (await slide.async("string")).replaceAll(
        "</a:t>",
        "</a:t>\u0000",
      );
      const slideText = xml
