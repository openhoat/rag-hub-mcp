...
function close(this: Store) {
  this.db.close()
}

function listKbs(this: Store): KbInfo[] {
  const sql = `
...
function listFiles(this: Store, kb: string): DocInfo[] {
  const sql = `
...
function getFile(this: Store, kbId: number, relPath: string): FileRecord | null {
  const row = this.db
...
function upsertFile(this: Store, rec: FileRecord): number {
...
function insertChunk(this: Store, rec: Omit<ChunkRecord, 'id'>): number {
...
function deleteChunks(this: Store, fileId: number) {
...
function getAllChunks(this: Store, kb?: string): ChunkRecord[] {
  if (kb) {
