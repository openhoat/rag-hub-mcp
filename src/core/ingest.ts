...
export const forceReindex = async (
  store: Store,
  queue: JobQueue,
  root: string = KB_ROOT,
...
export const scanAll = async (
  ...
  for (const kbName of kbDirs) {
    ...
    if (kbId)
      await scanKb(store, queue, kbId, kbName, root, knownFiles, result);
  }
...
const scanKb = async (
  ...
  for (const entry of entries) {
    ...
    const needsReembed = knownId
      ? await store.hasNullEmbeddings(knownId)
      : false;
  ...
  try {
    ...
    for (let i = 0; i < texts.length; i++) {
      try {
        ...
        logger.warn(
          "embed failed for chunk %d, storing without vector",
          i,
          perChunkErr,
        );
      }
