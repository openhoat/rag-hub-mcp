...
describe("addDocument", () => {
  ...
  test("should reject paths escaping the KB root", async () => {
    ...
    const queue = makeStubQueue();
    await expect(
      addDocument(store, queue, "docs", "../outside.md", "x", root),
    ).rejects.toThrow("invalid path");
    await expect(
      addDocument(store, queue, "docs", "a/../../../escape.md", "x", root),
    ).rejects.toThrow("invalid path");
  });
});
...
describe("deleteDocument", () => {
  ...
  test("should reject traversal paths", async () => {
    ...
    const queue = makeStubQueue();
    await expect(
      deleteDocument(store, queue, "docs", "../../etc/passwd", root),
    ).rejects.toThrow("invalid path");
  });
});
...
describe("readDocument", () => {
  ...
  test("should reject paths escaping the KB root", async () => {
    ...
    store = setup.store;
    await expect(
      readDocument("docs", "../../etc/passwd", root),
    ).rejects.toThrow("invalid path");
  });
});
